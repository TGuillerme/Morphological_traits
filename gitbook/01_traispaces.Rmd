...
## Adding node labels
continous_tree <- makeNodeLabel(continous_tree)
...
ancestral_snout_vent <- ace(snout_vent, continous_tree)
...
treats_data <- make.treats(continous_tree, data = snout_vent_data)
plot(treats_data)
```
...
## Get the node ages
node_ages <- tree.age(continous_tree)$age[-c(1:Ntip(continous_tree))]
...
```{r}
## Running the ancestral states for all the traits
ancestral_states <- multi.ace(continuous_data, continous_tree)
```
